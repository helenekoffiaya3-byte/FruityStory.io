...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (p[0] === "payments" && p[1] === "webhook" && method === "POST") {
      ...
      const signature = event.headers?.["stripe-signature"] || event.headers?.["Stripe-Signature"]; if (!signature) return json(400, { error: "Missing Stripe signature" });
      const raw = event.isBase64Encoded ? Buffer.from(event.body || "", "base64").toString("utf8") : (event.body || ""); const stripe = await stripeClient(); const stripeEvent = stripe.webhooks.constructEvent(raw, signature, secret);
    ...
    if (p[0] === "video" && p[1] === "generate" && method === "POST") {
      const body = readBody(event); const prompt = String(body.prompt || "").trim(); if (!prompt) return json(400, { error: "prompt is required" }); const cost = Math.max(1, Number(body.credits || process.env.VIDEO_API_CREDIT_COST || 100)); if (!Number.isFinite(cost) || cost > 100000) return json(400, { error: "Invalid credit cost" }); if (wallet.credits < cost) return json(402, { error: "Insufficient credits", balance: wallet.credits, required: cost });
