...
const getStripe = () => new Stripe(process.env.STRIPE_SECRET_KEY || process.env.STRIPE_RESTRICTED_KEY || process.env.PAYMENT_SECRET_KEY || '');

export const handler: Handler = async (event) => {
  if (event.httpMethod !== 'POST') return { statusCode: 405, body: 'Method Not Allowed' };
  if (!process.env.STRIPE_SECRET_KEY && !process.env.STRIPE_RESTRICTED_KEY && !process.env.PAYMENT_SECRET_KEY) return { statusCode: 500, body: JSON.stringify({ error: 'Stripe is not configured' }) };
  try {
