...
function getClipPrompts(body: any, prompt: string, maxScenes: number): string[] {
  ...
  const prompts = body.clips
    .map((clip: unknown) => {
      ...
      if (clip && typeof clip === "object" && typeof (clip as any).prompt === "string") {
        return (clip as any).prompt.trim();
      }
      return "";
    })
    .filter(Boolean);
...
export const handler: Handler = async (event) => {
  ...
  if (provider === "pixverse" && !process.env.PIXVERSE_API_KEY) {
    return json(500, { success: false, error: "PIXVERSE_API_KEY manquante côté serveur." });
  }
