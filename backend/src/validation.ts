...
export const generateSchema=z.object({prompt:z.string().trim().min(1).max(10000),duration:z.number().int().positive().max(1800).optional(),aspectRatio:z.enum(['9:16','16:9','1:1','4:5','3:4']).optional(),imageUrl:z.string().url().optional(),provider:z.enum(['auto','veo','seedance','pixverse','sora','openai']).optional()});
