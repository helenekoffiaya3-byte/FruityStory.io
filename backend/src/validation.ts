...
export const generateSchema=z.object({prompt:z.string().min(1).max(10000),duration:z.number().int().positive().max(1800).optional(),aspectRatio:z.string().max(20).optional(),imageUrl:z.string().url().optional(),provider:z.enum(['auto','veo','seedance','pixverse','sora']).optional()});
