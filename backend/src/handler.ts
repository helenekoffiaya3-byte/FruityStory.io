...
export const handler:Handler=async event=>{
 ...
 try{
  ...
  if(p[0]==='stripe'&&p[1]==='webhook'&&m==='POST'){
    ...
    const type=await handleStripeWebhook(raw,signature);
  ...
  if(p[0]==='auth'&&m==='POST'&&p[1]==='register'){
    const input=registerSchema.parse(body);const u=await createUser(input.username,input.email,await hashPassword(input.password));return response(201,{user:u,token:signToken({id:u.id,username:u.username})});
  }
  if(p[0]==='auth'&&m==='POST'&&p[1]==='login'){
    const input=loginSchema.parse(body);const u=await getUserByLogin(input.login);if(!u||!u.password_hash||!(await checkPassword(input.password,u.password_hash)))return response(401,{error:'Invalid credentials'});return response(200,{user:await getUser(u.id),token:signToken({id:u.id,username:u.username})});
  }
  ...
  if(p[0]==='subscriptions'){
    ...
    if(m==='GET'&&p[1]==='me'){
      const u=requireUser(event);const r=await db().query('SELECT id,plan_id AS "planId",status,current_period_end AS "currentPeriodEnd",cancel_at_period_end AS "cancelAtPeriodEnd" FROM subscriptions WHERE user_id=$1 ORDER BY updated_at DESC',[u.id]);
    ...
    if(m==='POST'&&p[1]==='checkout'){
      const u=requireUser(event);const planId=String(body.planId||'') as PlanId;
      if(!getPlan(planId))return response(400,{error:'Invalid planId',allowed:['standard','premium','pro','ultra_pro']});
      const user=await getUser(u.id);const result=await createSubscriptionCheckout(u.id,user?.email,planId);
      return response(200,{checkoutUrl:result.url,sessionId:result.id,plan:result.plan});
    }
    if(m==='POST'&&p[1]==='cancel'){
      const u=requireUser(event);const r=await db().query('SELECT stripe_subscription_id FROM subscriptions WHERE user_id=$1 AND status IN (\'active\',\'trialing\') ORDER BY updated_at DESC LIMIT 1',[u.id]);
      if(!r.rows[0])return response(404,{error:'No active subscription'});
      const { stripe } = await import('./stripe');
      const sub=await stripe().subscriptions.update(r.rows[0].stripe_subscription_id,{cancel_at_period_end:true});
      await db().query('UPDATE subscriptions SET cancel_at_period_end=true,updated_at=now() WHERE stripe_subscription_id=$1',[sub.id]);
      return response(200,{ok:true,cancelAtPeriodEnd:true});
    }
  }
