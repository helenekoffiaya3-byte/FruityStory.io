...
const response=(statusCode:number,body:unknown)=>({statusCode,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store','access-control-allow-origin':'*','access-control-allow-headers':'Content-Type, Authorization','access-control-allow-methods':'GET,POST,PATCH,DELETE,OPTIONS'},body:JSON.stringify(body)});
...
export const handler:Handler=async event=>{
 ...
 try{
  const p=path(event), m=event.httpMethod||'GET', body=jsonBody(event);
  ...
  if(p[0]==='payments'){requireUser(event);return response(501,{error:'Payment provider adapter must be configured before accepting money or webhooks.'});}
